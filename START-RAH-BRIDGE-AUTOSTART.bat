...
powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "$env:RAH_BRIDGE_HOST='127.0.0.1'; $env:RAH_BRIDGE_PORT='%BRIDGE_PORT%'; $env:RAH_JOB_DIR='C:\RAH\AgentJobs'; $env:RAH_JOB_REQUIRE_ADMIN='1'; $env:RAH_AI_AUTO_START_LMSTUDIO='1'; $env:RAH_AI_AUTO_START_ANYTHINGLLM='1'; Start-Process -FilePath '%VENV_PY%' -ArgumentList 'raven_bridge_agent.py' -WorkingDirectory '%BRIDGE_DIR%' -WindowStyle Minimized -RedirectStandardOutput '%BRIDGE_LOG%' -RedirectStandardError '%BRIDGE_LOG%.err'" >nul 2>nul
