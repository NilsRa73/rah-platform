...
fltmc >nul 2>&1
if "%ERRORLEVEL%"=="0" goto SYNC_MAIN
...
:PREPARE_LOCAL
set "RAH_SOURCE=%~dp0"
set "RAH_SOURCE_SHA=main"
goto VALIDATE

:SYNC_MAIN
echo [RAH] Loser siste main til en konkret commit-SHA...
...
set "RAH_SOURCE_SHA_FILE=%RAH_BOOT%\source-sha.txt"
if exist "%RAH_BOOT%" rmdir /s /q "%RAH_BOOT%" >nul 2>&1
mkdir "%RAH_STAGE%" >nul 2>&1
if errorlevel 1 goto FAIL_BOOT

powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop';$h=@{'User-Agent'='RAH-Raven-Final/2.0.1';'Accept'='application/vnd.github+json'};$u='https://api.github.com/repos/NilsRa73/rah-platform/commits/main';$sha=[string](Invoke-RestMethod -Uri $u -Headers $h -TimeoutSec 30).sha;if($sha.Length-ne40){throw 'Invalid main SHA'};$zip='https://github.com/NilsRa73/rah-platform/archive/'+$sha+'.zip';Invoke-WebRequest -UseBasicParsing -Uri $zip -OutFile $env:RAH_ZIP -TimeoutSec 90;Expand-Archive -LiteralPath $env:RAH_ZIP -DestinationPath $env:RAH_STAGE -Force;$again=[string](Invoke-RestMethod -Uri $u -Headers $h -TimeoutSec 30).sha;if($again-ne$sha){throw 'main moved during bootstrap; rerun FINAL'};$d=Get-ChildItem -LiteralPath $env:RAH_STAGE -Directory|Select-Object -First 1;if(-not$d){throw 'RAH archive missing project directory'};[IO.File]::WriteAllText($env:RAH_SOURCE_FILE,$d.FullName);[IO.File]::WriteAllText($env:RAH_SOURCE_SHA_FILE,$sha)"
if errorlevel 1 goto FAIL_DOWNLOAD
set /p RAH_SOURCE=<"%RAH_SOURCE_FILE%"
set /p RAH_SOURCE_SHA=<"%RAH_SOURCE_SHA_FILE%"
if not defined RAH_SOURCE goto FAIL_DOWNLOAD
if not defined RAH_SOURCE_SHA goto FAIL_DOWNLOAD
set "RAH_SOURCE=%RAH_SOURCE%\"
echo [RAH] Pinned main: %RAH_SOURCE_SHA%
