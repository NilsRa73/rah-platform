...
CANDIDATE_PATH=Path(__file__).with_name('rah-node-agent-v1.0-candidate.py')
_spec=importlib.util.spec_from_file_location('rah_node_agent_v10_candidate_pinned',CANDIDATE_PATH)
if _spec is None or _spec.loader is None:raise RuntimeError('Unable to load pinned Node Agent 1.0 Candidate implementation')
_impl=importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_impl)

# Stable identity only. Runtime authority and v5 behavior remain the already-gated Candidate implementation.
_impl.AGENT_VERSION='1.0.0'
AGENT_VERSION='1.0.0'
...
ROUTES=_impl.ROUTES
LocalConfirmationCoordinator=_impl.LocalConfirmationCoordinator
...
def make_handler(*args,**kwargs):
    ...
    handler.server_version='RAHNodeAgent/1.0'
    return handler
