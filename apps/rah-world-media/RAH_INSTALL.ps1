...
$Dst='C:\RAH\WorldMedia\9.9.1'
Write-Host 'RAH World Media 9.9.1 SMART PREVIEW - INSTALL' -ForegroundColor Yellow
...
foreach($lnk in @((Join-Path $Desktop 'RAH World Media 9.9.1.lnk'),(Join-Path $Programs 'RAH World Media 9.9.1.lnk'))){
  $s=$Wsh.CreateShortcut($lnk)
  $s.TargetPath=Join-Path $Dst 'START-HER.cmd'
  $s.WorkingDirectory=$Dst
  $s.Description='RAH World Media 9.9.1 SMART PREVIEW'
  $s.Save()
}
