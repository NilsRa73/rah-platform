...
$Dst='C:\RAH\WorldMedia\11.0'
Write-Host 'RAH World Media 11.0 RAVEN BROADCAST DECK - INSTALL' -ForegroundColor Yellow
...
foreach($lnk in @((Join-Path $Desktop 'RAH World Media 11.0.lnk'),(Join-Path $Programs 'RAH World Media 11.0.lnk'))){
  $s=$Wsh.CreateShortcut($lnk)
  $s.TargetPath=Join-Path $Dst 'START-HER.cmd'
  $s.WorkingDirectory=$Dst
  $s.Description='RAH World Media 11.0 RAVEN BROADCAST DECK'
  $s.Save()
}
