...
document.getElementById('rename').onclick=()=>{{let n=prompt('Receiver name',RNAME);if(n){{RNAME=n.trim().slice(0,40)||RNAME;localStorage.setItem('rahReceiverName',RNAME);deviceLabel(RNAME);heartbeat()}}}};heartbeat();poll();
    ...
    def open_media_wall(self):
        ...
        # Put obvious HLS manifests first so the first LIVE LIMIT bank fills quickly,
        # while opaque/other public stream URLs remain visible as standby tiles.
        world_all.sort(key=lambda c: 0 if ".m3u8" in c.url.lower() else 1)
...
function isHlsUrl(url){let u=String(url||'').trim();if(!/^https?:\/\//i.test(u))return false;if(/\.m3u8(?:[?#]|$)/i.test(u))return true;return !/\.(?:mpd|mp4|webm|mkv|avi|mov|mp3|aac|ogg)(?:[?#]|$)/i.test(u)}
