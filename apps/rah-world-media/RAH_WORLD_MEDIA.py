...
<script>const TOKEN={jt};async function clients(){{try{{let r=await fetch('/clients?token='+encodeURIComponent(TOKEN),{{cache:'no-store'}});if(!r.ok)return;let d=await r.json();let el=document.getElementById('clients');el.innerHTML='<b class="ok">'+d.count+' online</b>'+d.clients.map(x=>'<div class="client"><span>'+String(x.name||'Receiver').replace(/[&<>]/g,'')+'</span><span class="muted">'+Math.round(x.age_ms/1000)+'s • '+String(x.ip||'')+'</span></div>').join('')}}catch(e){{}}setTimeout(clients,2500)}}clients();document.addEventListener('keydown',e=>{{if(e.target&&['INPUT','TEXTAREA','SELECT'].includes(e.target.tagName))return;let k=e.key.toLowerCase();if(k==='1')openMosaic(16);else if(k==='2')openMosaic(36);else if(k==='3')openMosaic(64);else if(kk==='4')openMosaic(100);else if(kk==='5')openMosaic(256,true);else if(k==='6')openMosaic(500,true);else if(kk==='r')rotateLiveBank();else if(k==='x')toggleXreal();else if(k==='c')toggleClean();else if(k==='f')toggleFullscreen();else if(kk==='a')toggleAuto()}});
</script></body></html>'''
...
function nextHealthySource(exclude=''){{if(!mosaicSrc.length)return null;for(let i=0;i<mosaicSrc.length;i++){{let x=mosaicSrc[mosaicNext++%mosaicSrc.length];if(x&&x.url!==exclude&&sourceAvailable(x))return x}}return null}}
...
function updateMosaicStat(){{let e=document.getElementById('mosaicStat');if(e)e.textContent='VISIBLE '+mosaicCount+' • LIVE '+mosaicLive.length+'/'+liveLimit+' • OK '+mosaicHealthy+' • HEALED '+mosaicHealed+' • HOLD '+quarantine.size}}
...
function mosaicCols(n){{let aspect=document.body.classList.contains('xreal')?3.55:1.78;return Math.max(1,Math.ceil(Math.sqrt(n*aspect)))}}
...
function rotateLiveBank(){{let g=document.getElementById('mosaicGrid'),tiles=[...g.querySelectorAll('.mtile')];if(!tiles.length)return;if(tiles.length<=liveLimit){{nextMosaic();return}}mosaicLive.slice().forEach(detachMosaicVideo);let count=Math.min(liveLimit,tiles.length),start=(mosaicBank*count)%tiles.length;mosaicBank=(mosaicBank+1)%Math.max(1,Math.ceil(tiles.length/count));for(let i=0;i<count;i++){{let d=tiles[(start+i)%tiles.length],v=d.querySelector('video'),x=d._rahItem;if(v&&x)attachMosaicStream(d,v,x,0)}}updateMosaicStat()}}
function openMosaic(n,world=false){{clearMosaic();mosaicCount=Math.max(1,Math.min(500,n||mosaicCount));mosaicWorld=!!world;mosaicSrc=(mosaicWorld?DATA.world_tv:DATA.tv).filter(x=>x.url&&x.url.toLowerCase().includes('.m3u8'));if(!mosaicSrc.length){{alert('No browser-compatible HLS streams in this set. VLC playback may still work.');return}}if(mosaicOffset>=mosaicSrc.length)mosaicOffset=0;let rows=mosaicSrc.slice(mosaicOffset,mosaicOffset+mosaicCount);if(rows.length<mosaicCount)rows=rows.concat(mosaicSrc.slice(0,mosaicCount-rows.length));mosaicNext=(mosaicOffset+mosaicCount)%mosaicSrc.length;mosaicBank=1;let g=document.getElementById('mosaicGrid'),frag=document.createDocumentFragment();let cols=mosaicCols(rows.length);g.style.gridTemplateColumns=`repeat(${cols},1fr)`;g.style.gridTemplateRows=`repeat(${Math.ceil(rows.length/cols)},1fr)`;rows.forEach((x,i)=>{{let d=document.createElement('div');d.className='mtile standby';d._rahItem=x;d.innerHTML=`<span class="healthDot"></span><video muted playsinline autoplay></video><div class="mtitle">${esc(x.name)} • ${esc(x.country_name)}</div>`;frag.appendChild(d);let v=d.querySelector('video');d.onclick=()=>{{let current=d._rahItem||x;if(d.classList.contains('standby'))attachMosaicStream(d,v,current,0);g.querySelectorAll('video').forEach(z=>z.muted=true);v.muted=false;v.volume=.55}};if(i<Math.min(liveLimit,rows.length))setTimeout(()=>attachMosaicStream(d,v,x,0),Math.min(i*35,700))}});g.appendChild(frag);document.getElementById('mosaic').classList.add('open');startHealthTimer();updateMosaicStat()}}
