...
function detachMosaicVideo(entry){{if(!entry)return;let v=entry.video,t=entry.tile;try{{if(v._hls){{v._hls.destroy();v._hls=null}}v.pause();v.removeAttribute('src');v.load()}}catch(e){{}}t.classList.add('standby');t.classList.remove('recovering');mosaicLive=mosaicLive.filter(x=>x!==entry)}}
function clearMosaic(){{mosaicLive.slice().forEach(detachMosaicVideo);mosaicHls.forEach(h=>{{try{{h.destroy()}}catch(e){{}}}});mosaicHls=[];mosaicLive=[];let g=document.getElementById('mosaicGrid');g.innerHTML=''}}
function updateMosaicStat(){{let e=document.getElementById('mosaicStat');if(e)e.textContent='VISIBLE '+mosaicCount+' • LIVE '+mosaicLive.length+'/'+liveLimit}}
...
function toggleAuto(){{let b=document.getElementById('autoBtn');if(autoTimer){{clearInterval(autoTimer);autoTimer=null;b.textContent='AUTO 10s'}}else{{autoTimer=setInterval(nextMosaic,10000);b.textContent='AUTO ON'}}}}
...
function registerMosaicLive(tile,video,item){{let existing=mosaicLive.find(x=>x.video===video);if(existing)return existing;while(mosaicLive.length>=liveLimit)detachMosaicVideo(mosaicLive[0]);let entry={{tile:tile,video:video,item:item}};mosaicLive.push(entry);tile.classList.remove('standby');updateMosaicStat();return entry}}
function stopMosaic(){{clearMosaic();if(autoTimer){{clearInterval(autoTimer);autoTimer=null;document.getElementById('autoBtn').textContent='AUTO 10s'}}document.getElementById('mosaic').classList.remove('open')}}
function attachMosaicStream(tile,video,x,attempt=0){{registerMosaicLive(tile,video,x);tile.classList.remove('dead','standby');tile.classList.toggle('recovering',attempt>0); if(video._hls){{try{{video._hls.destroy()}}catch(e){{}};video._hls=null}} video.pause();video.removeAttribute('src');
...
function openMosaic(n,world=false){{clearMosaic();mosaicCount=Math.max(1,Math.min(500,n||mosaicCount));mosaicWorld=!!world;mosaicSrc=(mosaicWorld?DATA.world_tv:DATA.tv).filter(x=>x.url&&x.url.toLowerCase().includes('.m3u8'));if(!mosaicSrc.length){{alert('No browser-compatible HLS streams in this set. VLC playback may still work.');return}}if(mosaicOffset>=mosaicSrc.length)mosaicOffset=0;let rows=mosaicSrc.slice(mosaicOffset,mosaicOffset+mosaicCount);if(rows.length<mosaicCount)rows=rows.concat(mosaicSrc.slice(0,mosaicCount-rows.length));mosaicNext=(mosaicOffset+mosaicCount)%mosaicSrc.length;let g=document.getElementById('mosaicGrid');let cols=mosaicCols(rows.length);g.style.gridTemplateColumns=`repeat(${{cols}},1fr)`;g.style.gridTemplateRows=`repeat(${{Math.ceil(rows.length/cols)}},1fr)`;rows.forEach((x,i)=>{{let d=document.createElement('div');d.className='mtile standby';d.innerHTML=`<video muted playsinline autoplay></video><div class="mtitle">${{esc(x.name)}} • ${{esc(x.country_name)}}</div>`;g.appendChild(d);let v=d.querySelector('video');if(i<Math.min(liveLimit,rows.length))attachMosaicStream(d,v,x,0);d.onclick=()=>{{if(d.classList.contains('standby'))attachMosaicStream(d,v,x,0);g.querySelectorAll('video').forEach(z=>z.muted=true);v.muted=false;v.volume=.55}}}});document.getElementById('mosaic').classList.add('open');updateMosaicStat()}}
