...
function replaceMosaicSource(tile,video,failed,reason='fatal'){quarantineSource(failed&&failed.url);let entry=mosaicLive.find(x=>x.video===video);if(entry&&entry.healthy){entry.healthy=false;mosaicHealthy=Math.max(0,mosaicHealthy-1)}let replacement=nextHealthySource(failed&&failed.url||'');if(!replacement){if(entry)detachMosaicVideo(entry);tile.classList.add('dead');tile.classList.remove('recovering','healthy','standby');setTimeout(fillLiveSlots,80);updateMosaicStat();return}mosaicHealed++;tile._rahItem=replacement;let title=tile.querySelector('.mtitle');if(title)title.textContent=`${replacement.name} • ${replacement.country_name} • auto-healed`;setTimeout(()=>attachMosaicStream(tile,video,replacement,0),220);updateMosaicStat()}
function healthSweep(){if(document.visibilityState==='hidden')return;let now=Date.now();mosaicLive.slice().forEach(entry=>{let v=entry.video;if(!v||!entry.item)return;let t=Number(v.currentTime||0);if(v.readyState>=2&&t>entry.lastTime+.2){entry.lastTime=t;entry.progressAt=now;markMosaicHealthy(entry);return}if(now-entry.progressAt>14000){entry.progressAt=now;entry.tile.classList.add('recovering');replaceMosaicSource(entry.tile,v,entry.item,'stalled')}});updateMosaicStat()}
...
function attachMosaicStream(tile,video,x,attempt=0){if(!x||!sourceAvailable(x)){let replacement=nextHealthySource(x&&x.url||'');if(replacement)x=replacement;else{tile.classList.add('dead');return}}tile._rahItem=x;let entry=registerMosaicLive(tile,video,x);entry.item=x;entry.healthy=false;entry.progressAt=Date.now();tile.classList.remove('dead','standby','healthy');tile.classList.toggle('recovering',attempt>0);if(video._hls){try{let old=video._hls;mosaicHls=mosaicHls.filter(h=>h!==old);old.destroy()}catch(e){}video._hls=null}video.pause();video.removeAttribute('src');
  if(window.Hls&&Hls.isSupported()){let h=new Hls({enableWorker:true,maxBufferLength:4,maxMaxBufferLength:8,manifestLoadingTimeOut:5500,manifestLoadingMaxRetry:1,levelLoadingTimeOut:6000,levelLoadingMaxRetry:1,fragLoadingTimeOut:6500,fragLoadingMaxRetry:1});video._hls=h;mosaicHls.push(h);h.loadSource(x.url);h.attachMedia(video);h.on(Hls.Events.MANIFEST_PARSED,()=>{tile.classList.remove('recovering');video.play().catch(()=>{});markMosaicHealthy(entry)});h.on(Hls.Events.FRAG_LOADED,()=>markMosaicHealthy(entry));h.on(Hls.Events.ERROR,(_e,d)=>{if(!d||!d.fatal)return;if(attempt<1){entry.progressAt=Date.now();setTimeout(()=>attachMosaicStream(tile,video,x,attempt+1),450);return}replaceMosaicSource(tile,video,x,d.type||'fatal')})}
  else if(video.canPlayType('application/vnd.apple.mpegurl')){video.src=x.url;video.onplaying=()=>markMosaicHealthy(entry);video.play().catch(()=>{if(attempt<1)setTimeout(()=>attachMosaicStream(tile,video,x,attempt+1),550);else replaceMosaicSource(tile,video,x,'native')})} else replaceMosaicSource(tile,video,x,'unsupported')}
