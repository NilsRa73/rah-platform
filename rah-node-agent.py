...
import argparse,hmac,json,os,platform,secrets,shutil,socket,subprocess
from http.server import BaseHTTPRequestHandler,ThreadingHTTPServer
from pathlib import Path
AGENT_VERSION="0.5.0";PROTOCOL="rah-node-health-v1";STORAGE_PROTOCOL="rah-node-storage-v1";ACTIONS_PROTOCOL="rah-node-actions-v1";LAUNCH_PROTOCOL="rah-node-launch-v1";PORT=18766
...
def build_permissions(capabilities=None):
 ...
 return {"healthRead":True,"capabilityRead":True,"actionCatalogRead":True,"storageRead":"storage" in caps,"externalAppLaunch":"remote-desktop" in caps,"commands":False,"files":False,"shell":False,"remoteControl":False}
...
def build_actions_payload(capabilities=None,app_paths=None):
 ...
 for action_id in ("storage-summary.read","rustdesk.launch"):
  action=ACTION_CATALOG[action_id]
  if action["capability"] not in caps:continue
  if action_id=="rustdesk.launch" and not paths.get("rustdesk"):continue
...
def launch_executable(path):
 ...
 subprocess.Popen([path],**kwargs);return True
...
def make_handler(token,node_name="",node_role="",capabilities=None,app_paths=None,app_launcher=None):
 caps=sanitize_capabilities(capabilities);paths=build_app_paths(app_paths);health_payload=build_health_payload(node_name,node_role,caps);actions_payload=build_actions_payload(caps,paths);launcher=app_launcher or launch_executable
 class Handler(BaseHTTPRequestHandler):
  server_version="RAHNodeAgent/0.5";sys_version=""
  ...
  def _authorized(self):
   ...
   return True
  def do_OPTIONS(self):
   if self.path not in ("/health","/actions","/storage","/launch/rustdesk") or not self._origin_allowed():self._json(403,{"error":"forbidden"});return
   self.send_response(204);self._cors();self.send_header("Content-Length","0");self.end_headers()
  def do_GET(self):
   if self.path=="/launch/rustdesk":self._json(405,{"error":"method_not_allowed"});return
  ...
  def do_POST(self):
   ...
   if self.path!="/launch/rustdesk":self._json(404,{"error":"not_found"});return
   if not self._authorized():return
   if "remote-desktop" not in caps:self._json(403,{"error":"remote_desktop_capability_not_enabled"});return
   if int(self.headers.get("Content-Length","0") or 0)!=0:self._json(400,{"error":"request_body_not_allowed"});return
...
def main():
 ...
 print(f"RAH Node Agent v{AGENT_VERSION}");print("Mode: "+("LAN enrollment enabled" if args.allow_lan else "loopback only"));print(f"Port: {PORT}");print("Capabilities: "+(", ".join(capabilities) if capabilities else "identity-only"));print("Advertised actions: "+(", ".join(a["id"] for a in actions) if actions else "none"));print("RustDesk: "+("available for approved launch" if paths.get("rustdesk") else "not found in fixed locations/PATH"));print("Permissions: health/capability/action-catalog read; storage summary with storage capability; external app launch with remote-desktop capability; commands/files/shell/remote control disabled.");print("Enrollment/action token (memory only; changes on restart):");print(token);print("GET /health and GET /actions are authenticated. GET /storage remains fixed-volume and read-only.");print("POST /launch/rustdesk accepts no body, path or arguments and launches only the fixed RustDesk executable.");print("Command Center local approval is additionally required before CC invokes an advertised action.");print("No generic action/process endpoint, arbitrary path, installer, file listing, shell, command or remote-control endpoint exists.");print("Press Ctrl+C to stop the agent.")
