...
AGENT_VERSION="0.1.0";PROTOCOL="rah-node-health-v1";PORT=18766
...
def build_health_payload(node_name="",node_role=""):
 return {"protocol":PROTOCOL,"agentVersion":AGENT_VERSION,"status":"ready","hostname":clean_text(socket.gethostname(),80) or "Unknown host","platform":clean_text(platform.system(),80) or "Unknown platform","platformRelease":clean_text(platform.release(),80),"machine":clean_text(platform.machine(),40),"nodeName":clean_text(node_name,80),"nodeRole":clean_text(node_role,100)}
...
def make_handler(token,node_name="",node_role=""):
 class Handler(BaseHTTPRequestHandler):
  server_version="RAHNodeAgent/0.1";sys_version=""
  ...
  def do_GET(self):
   ...
   self._json(200,build_health_payload(node_name,node_role))
 ...
 return Handler
def create_server(host,port,token,node_name="",node_role=""):return ThreadingHTTPServer((host,port),make_handler(token,node_name,node_role))
def parse_args():
 p=argparse.ArgumentParser(description="RAH Node Agent — read-only health enrollment endpoint");p.add_argument("--allow-lan",action="store_true",help="Explicitly bind to LAN interfaces instead of loopback");p.add_argument("--name",default="");p.add_argument("--role",default="");return p.parse_args()
def main():
 args=parse_args();host="0.0.0.0" if args.allow_lan else "127.0.0.1";token=secrets.token_urlsafe(32);server=create_server(host,PORT,token,args.name,args.role)
 print(f"RAH Node Agent v{AGENT_VERSION}");print("Mode: "+("LAN enrollment enabled" if args.allow_lan else "loopback only"));print(f"Port: {PORT}");print("Enrollment token (memory only; changes on restart):");print(token);print("Only GET /health exists. No shell, commands, files or remote control endpoints are exposed.");print("Press Ctrl+C to stop the agent.")
