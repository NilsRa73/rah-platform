...
def parse_args():
 p=argparse.ArgumentParser(description="RAH Node Agent — read-only identity and capability endpoint")
 p.add_argument("--allow-lan",action="store_true",help="Explicitly bind to LAN interfaces instead of loopback")
 p.add_argument("--name",default="");p.add_argument("--role",default="")
 p.add_argument("--capability",action="append",choices=ALLOWED_CAPABILITIES,default=[],help="Explicitly advertise one read-only capability; repeat as needed")
 return p.parse_args()
def main():
