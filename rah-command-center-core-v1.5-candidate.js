(function(root,factory){
  const base=(typeof module==='object'&&module.exports)?require('./rah-command-center-core-v1.4.js'):(root&&root.RAHCommandCenterCoreV14);
  const api=factory(base);
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.RAHCommandCenterLocalProofCandidate=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(base){
'use strict';
if(!base)throw new Error('RAH Command Center 1.4 Stable core is required');

const CC_VERSION='1.5.0-candidate';
const NODE_ACTIONS_PROTOCOL='rah-node-actions-v5';
const ALLOWLIST_POLICY_ID='rah-capability-allowlist-v1';
const APPROVAL_ACTION_HEADER='X-RAH-Approval-Action';
const APPROVAL_TARGET_HEADER='X-RAH-Approval-Target';
const LOCAL_APPROVAL_HEADER='X-RAH-Local-Approval';
const ACTION_CHALLENGE_HEADER=base.ACTION_CHALLENGE_HEADER;
const V5_CHALLENGE_TTL_SECONDS=30;
...
function validateActionRow(raw,caps){
  ...
  const challengePresent=raw.challenge!==undefined;
  const proofPresent=raw.localApprovalProof!==undefined;
  const challenge=challengePresent?base.sanitizeActionChallenge(raw.challenge):'';
  const proof=proofPresent?sanitizeProof(raw.localApprovalProof):'';
  if(challengePresent&&(!challenge||raw.challengeTtlSeconds!==V5_CHALLENGE_TTL_SECONDS))return null;
...
function localApprovalGrantFromCatalog(payload,capabilities,actionId,expectedSessionId){
  ...
  const catalog=sanitizeActionCatalog(payload,capabilities,expectedSessionId);
  if(!catalog||!catalog.actions.includes(actionId))return null;
  const caps=base.sanitizeCapabilities(capabilities);
  for(const raw of payload.actions){
    const row=validateActionRow(raw,caps);
    if(row&&row.id===actionId&&row.challenge&&row.proof&&row.challenge!==row.proof)return Object.freeze({actionId,challenge:row.challenge,localApprovalProof:row.proof,challengeTtlSeconds:V5_CHALLENGE_TTL_SECONDS,localApprovalProofTtlSeconds:LOCAL_APPROVAL_TTL_SECONDS});
  }
  return null;
}
function normalizeVerifiedCatalog(value,capabilities){
  if(!isPlainObject(value)||value.protocol!==NODE_ACTIONS_PROTOCOL||value.status!=='ready'||value.policyId!==ALLOWLIST_POLICY_ID||value.approvalMode!=='command-center-ephemeral-plus-node-local')return null;
  ...
  return sessionId?{protocol:NODE_ACTIONS_PROTOCOL,status:'ready',policyId:ALLOWLIST_POLICY_ID,approvalMode:'command-center-ephemeral-plus-node-local',sessionId,actions}:null;
}
...
function localApprovalIntentRequest(record,actionId,target){
  if(!MUTATING_ACTION_IDS.includes(actionId)||!base.canExecuteAction(record,actionId))return null;
  const url=base.nodeActionsUrl(record.endpointIp);if(!url)return null;
  const headers={[APPROVAL_ACTION_HEADER]:actionId};
...
function sanitizeGrant(grant,actionId){
  ...
  if(!challenge||!proof||challenge===proof||grant.challengeTtlSeconds!==V5_CHALLENGE_TTL_SECONDS||grant.localApprovalProofTtlSeconds!==LOCAL_APPROVAL_TTL_SECONDS)return null;
  return{challenge,localApprovalProof:proof};
}
function actionExecutionRequest(record,actionId,value){
  ...
  const grant=sanitizeGrant(value,actionId),url=base.nodeRustDeskLaunchUrl(record.endpointIp);
  return grant&&url?{id:actionId,url,method:'POST',mutating:true,scope:catalog.scope,headers:Object.freeze({[ACTION_CHALLENGE_HEADER]:grant.challenge,[LOCAL_APPROVAL_HEADER]:grant.localApprovalProof})}:null;
}
function rustDeskHandoffRequest(record,peerId,grantValue){
  if(!base.canExecuteAction(record,'rustdesk.connect'))return null;
  const id=base.sanitizePeerId(peerId),grant=sanitizeGrant(grantValue,'rustdesk.connect'),url=base.nodeRustDeskHandoffUrl(record.endpointIp);
  return id&&grant&&url?{id:'rustdesk.connect',url,method:'POST',headers:Object.freeze({[ACTION_CHALLENGE_HEADER]:grant.challenge,[LOCAL_APPROVAL_HEADER]:grant.localApprovalProof}),body:{peerId:id}}:null;
}
return Object.freeze({...base,CC_VERSION,NODE_ACTIONS_PROTOCOL,ALLOWLIST_POLICY_ID,APPROVAL_ACTION_HEADER,APPROVAL_TARGET_HEADER,LOCAL_APPROVAL_HEADER,ACTION_CHALLENGE_HEADER,V5_CHALLENGE_TTL_SECONDS,LOCAL_APPROVAL_TTL_SECONDS,MUTATING_ACTION_IDS,sanitizeActionCatalog,actionChallengeFromCatalog,localApprovalGrantFromCatalog,normalizeVerifiedCatalog,enrollDevice,actionChallengeRequest,localApprovalIntentRequest,actionExecutionRequest,rustDeskHandoffRequest});
});
