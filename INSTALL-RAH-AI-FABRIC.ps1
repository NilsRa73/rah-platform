param(
    ...
    [ValidateScript({ $_ -eq "main" -or $_ -match '^[0-9a-fA-F]{40}
...
$ArchiveUrl = if($Ref -match '^[0-9a-fA-F]{40}
...
function Elevate-IfNeeded {
    ...
    Start-Process powershell.exe -Verb RunAs -ArgumentList @(
        "-NoProfile","-ExecutionPolicy","Bypass","-File","`"$PSCommandPath`"","-Mode",$Mode,"-Ref",$Ref
    )
    exit
}
...
if($Mode-ne"Status"){Read-Host "Trykk ENTER for a avslutte"}
    ...
    "https://github.com/NilsRa73/rah-platform/archive/refs/heads/$Ref.zip"
}
$BridgeTask = "RAH Raven Bridge"
$NodeTask = "RAH Raven Node Agent 18766"
$ProviderTask = "RAH Raven AI Providers"
