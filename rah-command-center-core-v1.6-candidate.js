...
const CONTEXT_MIN_LENGTH=32;
const CONTEXT_MAX_LENGTH=128;

function isPlainObject(v){return !!v&&typeof v==='object'&&!Array.isArray(v)}
function sanitizeRequesterContext(v){return typeof v==='string'&&v===v.trim()&&v.length>=CONTEXT_MIN_LENGTH&&v.length<=CONTEXT_MAX_LENGTH&&/^[A-Za-z0-9_-]+$/.test(v)?v:''}
...
function validateActionRow(raw,caps){
  ...
  if(challengePresent&&(!challenge||raw.challengeTtlSeconds!==base.V5_CHALLENGE_TTL_SECONDS))return null;
  if(!challengePresent&&raw.challengeTtlSeconds!==undefined)return null;
  if(proofPresent&&(!catalog.mutating||!proof||raw.localApprovalProofTtlSeconds!==base.LOCAL_APPROVAL_TTL_SECONDS))return null;
  ...
  if(catalog.mutating&&challengePresent&&challenge===proof)return null;
...
function localApprovalGrantFromCatalog(payload,capabilities,actionId,expectedSessionId){
  ...
  const caps=base.sanitizeCapabilities(capabilities);
  for(const raw of payload.actions){const row=validateActionRow(raw,caps);if(row&&row.id===actionId&&row.challenge&&row.proof&&row.challenge!==row.proof)return Object.freeze({actionId,challenge:row.challenge,localApprovalProof:row.proof,challengeTtlSeconds:base.V5_CHALLENGE_TTL_SECONDS,localApprovalProofTtlSeconds:base.LOCAL_APPROVAL_TTL_SECONDS})}
  return null;
}
...
function actionChallengeRequest(record,actionId){
  ...
  const url=base.nodeActionsUrl(record.endpointIp);return url?{id:actionId,url,method:'GET',protocol:NODE_ACTIONS_PROTOCOL,policyId:ALLOWLIST_POLICY_ID,headers:Object.freeze({})}:null;
}
function localApprovalIntentRequest(record,actionId,target,requesterContext){
  ...
  const headers={[base.APPROVAL_ACTION_HEADER]:actionId,[REQUESTER_CONTEXT_HEADER]:context};
  if(actionId==='rustdesk.launch'){
    if(target!==undefined&&target!==null&&target!=='')return null;
  }else{
    const peerId=base.sanitizePeerId(target);if(!peerId)return null;headers[base.APPROVAL_TARGET_HEADER]=peerId;
  }
...
function sanitizeGrant(grant,actionId){
  ...
  if(!challenge||!proof||challenge===proof||grant.challengeTtlSeconds!==base.V5_CHALLENGE_TTL_SECONDS||grant.localApprovalProofTtlSeconds!==base.LOCAL_APPROVAL_TTL_SECONDS)return null;
...
function actionExecutionRequest(record,actionId,value,requesterContext){
  ...
  if(actionId==='storage-summary.read'){
    ...
    return challenge&&url?{id:actionId,url,method:'GET',mutating:false,scope:catalog.scope,headers:Object.freeze({[base.ACTION_CHALLENGE_HEADER]:challenge})}:null;
  }
  ...
  return context&&grant&&url?{id:actionId,url,method:'POST',mutating:true,scope:catalog.scope,headers:Object.freeze({[base.ACTION_CHALLENGE_HEADER]:grant.challenge,[base.LOCAL_APPROVAL_HEADER]:grant.localApprovalProof,[REQUESTER_CONTEXT_HEADER]:context})}:null;
}
function rustDeskHandoffRequest(record,peerId,grantValue,requesterContext){
  ...
  return id&&context&&grant&&url?{id:'rustdesk.connect',url,method:'POST',headers:Object.freeze({[base.ACTION_CHALLENGE_HEADER]:grant.challenge,[base.LOCAL_APPROVAL_HEADER]:grant.localApprovalProof,[REQUESTER_CONTEXT_HEADER]:context}),body:{peerId:id}}:null;
}
return Object.freeze({...base,CC_VERSION,NODE_ACTIONS_PROTOCOL,ALLOWLIST_POLICY_ID,REQUESTER_CONTEXT_HEADER,MUTATING_ACTION_IDS,CONTEXT_MIN_LENGTH,CONTEXT_MAX_LENGTH,sanitizeRequesterContext,sanitizeActionCatalog,actionChallengeFromCatalog,localApprovalGrantFromCatalog,normalizeVerifiedCatalog,enrollDevice,actionChallengeRequest,localApprovalIntentRequest,actionExecutionRequest,rustDeskHandoffRequest});
});
