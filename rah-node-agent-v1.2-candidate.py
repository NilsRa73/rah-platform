...
import hashlib,hmac,importlib.util,ipaddress,json,queue,secrets,sys,threading,time
...
_source_impl=_stable._impl
_base=_source_impl._base
...
def valid_requester_context(value):
    return isinstance(value,str) and 32<=len(value)<=128 and all(ch.isalnum() or ch in '_-' for ch in value)
...
def build_actions_payload(capabilities=None,app_paths=None,session_id=''):
    ...
    for raw in payload.get('actions',[]):
        ...
        if item.get('id') in MUTATING_ACTION_IDS:
            ...
            item.pop('requesterContextRequired',None)
        rows.append(item)
...
class RequesterContextConfirmationCoordinator(_stable.SourceBoundConfirmationCoordinator):
    def request(self,action_id,target='',requester_source='',requester_context=''):
        ...
        intent={
            ...
            'requesterContextDigest':context_digest,
            'event':event,
            'result':None,
            'cancelled':False,
    ...
    def _finish(self,intent,approved):
        ...
        with self._lock:
            ...
            if intent.get('cancelled') or now>float(intent.get('expires',0)):
                ...
                intent['event'].set();return
            if approved is not True:
                intent['result']={'ok':False,'error':'local_confirmation_denied'}
                intent['event'].set();return
            source=normalize_requester_source(intent.get('requesterSource',''))
            context_digest=str(intent.get('requesterContextDigest',''))
            if not source:
                intent['result']={'ok':False,'error':'requester_source_not_allowed'}
                intent['event'].set();return
            if not context_digest or len(context_digest)!=64:
                intent['result']={'ok':False,'error':'requester_context_invalid'}
                intent['event'].set();return
    ...
    def consume(self,action_id,target,challenge,proof,requester_source='',requester_context=''):
        ...
        with self._lock:
            ...
            if now>float(pair.get('expires',0)):
                self._active_pair=None;return 'expired'
    ...
    def snapshot(self):
        ...
        with self._lock:
            ...
            if pending is not None and snapshot.get('pending') is not None:
                snapshot['pending']['requesterContextDigest']=pending.get('requesterContextDigest','')
            if pair is not None and snapshot.get('activePair') is not None:
                snapshot['activePair']['requesterContextDigest']=pair.get('requesterContextDigest','')
        return snapshot
...
def consume_read_challenge(state,lock,action_id,value,now=None):
    return _stable.consume_read_challenge(state,lock,action_id,value,now)
...
def make_handler(token,node_name='',node_role='',capabilities=None,app_paths=None,app_launcher=None,handoff_launcher=None,challenge_ttl_seconds=ACTION_CHALLENGE_TTL_SECONDS,session_id=None,coordinator=None,interactive_console=None,local_input=None,clock=None,token_func=None):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _forbid_requester_context(self):
            ...
            if self.headers.get(REQUESTER_CONTEXT_HEADER) is not None:
                self._json(400,{'error':'requester_context_not_allowed'});return False
            return True
        ...
        def _approval_headers_valid(self):
            ...
            for key in self.headers.keys():
                lowered=key.lower()
                if lowered.startswith('x-rah-approval-') and lowered not in allowed:self._json(400,{'error':'unknown_approval_header'});return False
            if self.headers.get(LOCAL_APPROVAL_HEADER) is not None or self.headers.get(ACTION_CHALLENGE_HEADER) is not None:
                self._json(400,{'error':'approval_grant_header_not_allowed'});return False
            return self._requester_header_valid()
        ...
        def _require_local_pair(self,action_id,target=''):
            ...
            errors={
                'missing':(428,'local_approval_proof_required'),'expired':(409,'local_approval_proof_expired'),
                'requester_source_not_allowed':(403,'requester_source_not_allowed'),'requester_mismatch':(409,'local_approval_requester_mismatch'),
                'requester_context_invalid':(400,'requester_context_invalid'),'requester_context_mismatch':(409,'local_approval_requester_context_mismatch'),
                'action_mismatch':(409,'local_approval_action_mismatch'),'session_mismatch':(409,'local_approval_session_mismatch'),
                'input_mismatch':(409,'local_approval_input_mismatch'),'challenge_invalid':(409,'action_challenge_invalid_or_expired'),
        ...
        def do_OPTIONS(self):
            if self.path not in ROUTES or not self._origin_allowed():self._json(403,{'error':'forbidden'});return
        ...
        def do_POST(self):
            ...
            if self.path=='/launch/rustdesk':
                if self.headers.get('Transfer-Encoding'):self._json(400,{'error':'request_body_not_allowed'});return
                try:length=int(self.headers.get('Content-Length','0') or 0)
                except ValueError:self._json(400,{'error':'invalid_content_length'});return
                if length!=0:self._json(400,{'error':'request_body_not_allowed'});return
                ...
                self._json(200,{'protocol':_base.LAUNCH_PROTOCOL,'status':'launched','app':'rustdesk'});return
...
def main():
    args=_base.parse_args();host='0.0.0.0' if args.allow_lan else '127.0.0.1';token=secrets.token_urlsafe(32)
    capabilities=sanitize_capabilities(args.capability);paths=build_app_paths()
    ...
    print('Requester context: fixed ephemeral header; Node stores digest only in active mutating pair')
    print('Requester source binding: actual IPv4 socket peer only; forwarding headers ignored')
    print('Mode: '+('LAN enrollment enabled' if args.allow_lan else 'loopback only'));print(f'Port: {PORT}')
    print('Capabilities: '+(', '.join(capabilities) if capabilities else 'identity-only'));print('Advertised actions: '+(', '.join(a['id'] for a in actions) if actions else 'none'))
    ...
    print('Token: '+token);print('Boundary: fixed 4 capabilities / 3 actions / 5 routes; no shell/files/generic process/native remote-control API')
