...
def main() -> None:
    ...
    require(text, "const ids=x.devices.map(d=>d&&d.id)", "enhets-ID-er fra lagret/importert tilstand")

    # Persisted/imported device names must follow the same normalized uniqueness
    # rule already used when adding a device in the UI.
    require(text, "function uniqueDeviceNames(x)", "unik normalisert enhetsnavn-validering")
    require(text, "x.devices.map(d=>d&&normalizeName(d.name))", "lagrede/importerte navn normaliseres")
    require(text, "uniqueDeviceIds(x)&&uniqueDeviceNames(x)&&", "backupvalidering krever unike enhets-ID-er og navn")
    ...
    require(text, "ip:ip||'Ikke satt'", "tom IP lagres som Ikke satt")
