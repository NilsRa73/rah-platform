...
const files=m.package_files;
function updaterAllowlist(){const x=up.match(/\$AllowedPackageFiles=@\(\s*([\s\S]*?)\n\)/);assert.ok(x,'updater AllowedPackageFiles block');return[...x[1].matchAll(/"([^"]+)"/g)].map(v=>v[1])}

test('canonical v2.4 Stable generation 9 package closure',()=>{
  assert.equal(m.version,'2.4.0');assert.equal(m.stage,'stable');
  assert.equal(m.entry,'RAH-COMMAND-CENTER-V2.4.html');assert.equal(m.runtime,'rah-command-center-core-v2.4.js');
  assert.equal(m.previous_stable_version,'2.3.0');assert.equal(m.stable_release_manifest,'RAH-CC24-NODE14-STABLE-RELEASE.json');
  assert.equal(m.canonical_package_generation,9);assert.equal(m.features.canonical_package_dependency_count,71);
  assert.equal(m.release_gate.status,'passed');assert.equal(m.release_gate.runtime_files_frozen,true);assert.equal(m.development_paused,true);
  assert.equal(files.length,71);assert.equal(new Set(files).size,71);for(const f of files)assert.ok(fs.existsSync(f),f);
