...
const html = fs.readFileSync(path.join(root, 'RAH-COMMAND-CENTER-V0.4.html'), 'utf8');
...
const devices = core.buildDeviceSnapshot(null);
assert.equal(devices.totalCount, 4);
assert.equal(devices.remoteControlCount, 0);
assert.equal(devices.commandCount, 0);
...
assert.match(html, /Not checked\. CC v0\.4 never probes the Bridge until you click the button\./);
...
assert.doesNotMatch(html, /password|token|api[_ -]?key/i);

console.log('RAH Command Center v0.4 integration gate passed');
