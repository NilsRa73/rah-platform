...
class NodeAgentTests(unittest.TestCase):
 def setUp(self):
  self.token='test-token-abcdefghijklmnopqrstuvwxyz';self.server=agent.create_server('127.0.0.1',0,self.token,'Test Node','Read only',['compute','shell','storage','compute']);self.port=self.server.server_address[1];self.thread=threading.Thread(target=self.server.serve_forever,daemon=True);self.thread.start()
 ...
 def test_capability_allowlist_is_explicit(self):self.assertEqual(agent.sanitize_capabilities(['compute','SHELL','display','remote-desktop','storage','compute']),['compute','display','remote-desktop','storage']);self.assertEqual(agent.sanitize_capabilities(None),[]);self.assertEqual(agent.ALLOWED_CAPABILITIES,('compute','storage','display','remote-desktop'))
 def test_fixed_public_contract(self):self.assertEqual(agent.PORT,18766);self.assertEqual(agent.PROTOCOL,'rah-node-health-v1');self.assertNotIn('*',agent.ALLOWED_ORIGINS);self.assertEqual(agent.READ_ONLY_PERMISSIONS['commands'],False);self.assertEqual(agent.READ_ONLY_PERMISSIONS['files'],False);self.assertEqual(agent.READ_ONLY_PERMISSIONS['shell'],False);self.assertEqual(agent.READ_ONLY_PERMISSIONS['remoteControl'],False)
