...
class NodeAgentTests(unittest.TestCase):
    def setUp(self):
        ...
        self.server=agent.create_server('127.0.0.1',0,self.token,'Test Node','Read only')
    ...
    def test_health_requires_bearer_token(self):
        ...
        self.assertEqual(payload['protocol'],'rah-node-health-v1')
        self.assertEqual(payload['status'],'ready')
        self.assertEqual(payload['nodeName'],'Test Node')
        self.assertNotIn('token',payload)
        self.assertNotIn('username',payload)

    def test_only_health_get_is_exposed(self):
        status,_,_=self.request('/command',token=self.token)
        self.assertEqual(status,404)
        status,_,_=self.request('/health',token=self.token,method='POST')
        self.assertEqual(status,405)
        status,_,_=self.request('/files',token=self.token)
        self.assertEqual(status,404)

    def test_origin_allowlist_and_private_network_preflight(self):
        status,_,_=self.request(token=self.token,origin='https://evil.example')
        self.assertEqual(status,403)
        status,headers,_=self.request(method='OPTIONS',private_network=True)
    ...
    def test_authorization_comparison(self):
        self.assertTrue(agent.is_authorized('Bearer '+self.token,self.token))
        self.assertFalse(agent.is_authorized('Bearer wrong',self.token))
        self.assertFalse(agent.is_authorized(None,self.token))

    def test_fixed_public_contract(self):
        self.assertEqual(agent.PORT,18766)
        self.assertEqual(agent.PROTOCOL,'rah-node-health-v1')
        self.assertNotIn('*',agent.ALLOWED_ORIGINS)
