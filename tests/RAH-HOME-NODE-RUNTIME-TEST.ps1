...
$p=$null
function Run-Client([string[]]$Args){$o=& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $client @Args 2>&1;$code=$LASTEXITCODE;[pscustomobject]@{Code=$code;Text=($o|Out-String)}}
try{
 ...
 $hello=Run-Client @('-NodeAddress','127.0.0.1','-Port',"$port",'-Action','hello');if($hello.Code-ne0-or$hello.Text-notmatch 'RAH Home Node Agent'){throw "hello failed: $($hello.Text)"}
 $wrong=Run-Client @('-NodeAddress','127.0.0.1','-Port',"$port",'-Action','pair','-PairCode','000000');if($wrong.Code-eq0-or$wrong.Text-notmatch 'pair-code-rejected'){throw "wrong pair was not rejected: $($wrong.Text)"}
 $pair=Run-Client @('-NodeAddress','127.0.0.1','-Port',"$port",'-Action','pair','-PairCode',$code);if($pair.Code-ne0){throw "pair failed: $($pair.Text)"}
 foreach($action in @('health','systemInfo','benchmark')){$r=Run-Client @('-NodeAddress','127.0.0.1','-Port',"$port",'-Action',$action);if($r.Code-ne0-or$r.Text-notmatch '"ok"\s*:\s*true'){throw "$action failed: $($r.Text)"}}
 ...
 $unauth=Run-Client @('-NodeAddress','127.0.0.1','-Port',"$port",'-Action','health');if($unauth.Code-eq0-or$unauth.Text-notmatch 'unauthorized'){throw "unauthorized request was not rejected: $($unauth.Text)"}
