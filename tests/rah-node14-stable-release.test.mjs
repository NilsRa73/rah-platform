...
for(const row of [release.runtime.nodeAgent,release.runtime.windowsStarter,release.runtime.linuxStarter,release.pinnedCandidate.nodeAgent,release.directRollback.nodeAgent,release.directRollback.release]){
...
const bat=fs.readFileSync(path.join(ROOT,'START-RAH-NODE-AGENT.bat'),'utf8');
for(const marker of ['RAH NODE AGENT v1.4 STABLE','rah-node-agent-v1.4.py','rah-node-agent-v1.4-candidate.py','raw.githubusercontent.com/NilsRa73/rah-platform/main','127.0.0.1:18765','HMAC-SHA256'])assert.ok(bat.includes(marker),`Windows starter missing ${marker}`);
assert.equal(/Invoke-Expression|\biex\b|DownloadString/i.test(bat),false,'Windows starter must not dynamically execute downloaded text');

const sh=fs.readFileSync(path.join(ROOT,'START-RAH-NODE-AGENT.sh'),'utf8');
for(const marker of ['RAH NODE AGENT v1.4 STABLE','rah-node-agent-v1.4.py','rah-node-agent-v1.4-candidate.py','raw.githubusercontent.com/NilsRa73/rah-platform/main','127.0.0.1:18765'])assert.ok(sh.includes(marker),`Linux starter missing ${marker}`);
