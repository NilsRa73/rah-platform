...
assert.equal(privacy.raven_now_local_bridge_only,true,"Raven Now Bridge requests must be loopback-only");
...
assert.equal(privacy.raven_studio_status_polling_loopback_only,true,"Raven Studio status polling must stay loopback-only");
...
assert.equal(privacy.raven_studio_stable,true,"Raven Studio v2.8 stable marker must stay true");
assert.equal(privacy.agent_runner_stable,true,"Agent Runner v0.3 stable marker must stay true");
assert.ok(manifest.files.includes("RAH-RAVEN-STUDIO-VERSION.json"),"Raven Studio component manifest must ship in Raven package");
const studioManifest=JSON.parse(read("RAH-RAVEN-STUDIO-VERSION.json"));
assert.equal(studioManifest.version,"2.8.0");
assert.equal(studioManifest.stage,"stable");
...
assert.equal(studioManifest.next_milestone,null);
...
assert.equal(studioManifest.stable_release_gate?.runtime_files_frozen,true);
assert.equal(manifest.release_gate.stable_components.raven_studio,"2.8","Raven Studio v2.8 must be stable");
assert.equal(manifest.release_gate.bugfix_component_updates.raven_studio,"2.8","Raven Studio v2.8 bugfix pin must be recorded");

assert.ok(manifest.files.includes("RAH-RAVEN-NOW-VERSION.json"),"Raven Now component manifest must ship in Raven package");
const nowManifest=JSON.parse(read("RAH-RAVEN-NOW-VERSION.json"));
assert.equal(nowManifest.version,"2.17.0");
assert.equal(nowManifest.stage,"stable");
...
assert.equal(nowManifest.next_milestone,null);
...
assert.equal(nowManifest.stable_release_gate?.runtime_files_frozen,true);
...
assert.equal(manifest.release_gate?.bugfix_component_updates?.raven_studio,"2.8");
assert.equal(manifest.release_gate?.bugfix_component_updates?.raven_now,"2.17");
