...
class LocalConfirmationCoordinator:
    def __init__(self,session_id,interactive=None,input_func=None,clock=None,token_func=None,wait_seconds=LOCAL_CONFIRMATION_WAIT_SECONDS,proof_ttl_seconds=LOCAL_APPROVAL_TTL_SECONDS,cooldown_seconds=LOCAL_CONFIRMATION_COOLDOWN_SECONDS,start_thread=True):
        ...
        self.wait_seconds=max(1,float(wait_seconds))
        self.proof_ttl_seconds=max(1,float(proof_ttl_seconds))
    ...
    def request(self,action_id,target=''):
        ...
        digest=canonical_input_digest(action_id,target)
        if not digest:return {'ok':False,'error':'invalid_fixed_action_input'}
        now=float(self.clock())
        event=threading.Event()
        intent={'actionId':action_id,'inputDigest':digest,'displayTarget':target if action_id=='rustdesk.connect' else '', 'event':event,'result':None,'cancelled':False,'expires':now+self.wait_seconds}
        with self._lock:
    ...
    def _finish(self,intent,approved):
        ...
        with self._lock:
            ...
            if not challenge or not proof or _safe_equal(challenge,proof):
                ...
                intent['event'].set();return
            self._active_pair={'actionId':intent['actionId'],'sessionId':self.session_id,'inputDigest':intent['inputDigest'],'challenge':challenge,'proof':proof,'expires':now+self.proof_ttl_seconds}
    ...
    def consume(self,action_id,target,challenge,proof):
        ...
        with self._lock:
            ...
            if not digest or digest!=pair.get('inputDigest'):return 'input_mismatch'
    ...
    def snapshot(self):
        with self._lock:
            ...
            return {
                ...
                'pending':None if pending is None else {'actionId':pending['actionId'],'inputDigest':pending['inputDigest'],'cancelled':bool(pending.get('cancelled'))},
                'activePair':None if pair is None else {'actionId':pair['actionId'],'sessionId':pair['sessionId'],'inputDigest':pair['inputDigest'],'expires':pair['expires']},
    ...
    with challenge_lock:
        ...
        for raw in base_payload.get('actions',[]):
            ...
            if action_id=='storage-summary.read':
                challenge=_token();challenge_state[action_id]={'value':challenge,'expires':ts+ttl};item['challenge']=challenge;item['challengeTtlSeconds']=ttl
            ...
            actions.append(item)
    return {'protocol':ACTIONS_PROTOCOL,'status':'ready','sessionId':base_payload.get('sessionId',''),'policyId':ALLOWLIST_POLICY_ID,'actions':actions,'approvalMode':'command-center-ephemeral-plus-node-local'}
...
def make_handler(token,node_name='',node_role='',capabilities=None,app_paths=None,app_launcher=None,handoff_launcher=None,challenge_ttl_seconds=ACTION_CHALLENGE_TTL_SECONDS,session_id=None,coordinator=None,interactive_console=None,local_input=None,clock=None,token_func=None):
    ...
    health_payload=build_health_payload(node_name,node_role,caps,session);actions_payload=build_actions_payload(caps,paths,session)
    challenge_state={};challenge_lock=threading.Lock();launcher=app_launcher or _base.launch_executable;handoff=handoff_launcher or _base.launch_rustdesk_connect
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version='RAHNodeAgent/1.0-candidate';sys_version=''
        ...
        def _approval_intent(self):
            ...
            if action is None:
                ...
                return True,None
            if self.headers.get('Transfer-Encoding') or int(self.headers.get('Content-Length','0') or 0)!=0:self._json(400,{'error':'approval_intent_body_not_allowed'});return False,None
            ...
            if action=='rustdesk.launch':
                ...
                if not is_valid_rustdesk_peer_id(target):self._json(400,{'error':'invalid_peer_id'});return False,None
        ...
        def _require_local_pair(self,action_id,target=''):
            challenge=self.headers.get(ACTION_CHALLENGE_HEADER,'');proof=self.headers.get(LOCAL_APPROVAL_HEADER,'')
            ...
            errors={
                'missing':(428,'local_approval_proof_required'),'expired':(409,'local_approval_proof_expired'),'action_mismatch':(409,'local_approval_action_mismatch'),'session_mismatch':(409,'local_approval_session_mismatch'),'input_mismatch':(409,'local_approval_input_mismatch'),'challenge_invalid':(409,'action_challenge_invalid_or_expired'),'proof_invalid':(409,'local_approval_proof_invalid')
            }
            if result=='ok':return True
        ...
        def do_POST(self):
            ...
            if self.path=='/launch/rustdesk':
                if self.headers.get('Transfer-Encoding'):
                    self._json(400,{'error':'request_body_not_allowed'});return
                try:length=int(self.headers.get('Content-Length','0') or 0)
