...
function Resolve-VerifiedRepositoryCommit {
    ...
    $commitInfo = Invoke-RestMethod -UseBasicParsing -Headers $headers -Uri "$ApiBase/commits/$RepoBranch"
...
try {
    Write-CcLog "Starter RAH Command Center verifisert pakkeoppdatering."

    $ResolvedCommit = Resolve-VerifiedRepositoryCommit
    $RawBase = "https://raw.githubusercontent.com/$RepoOwner/$RepoName/$ResolvedCommit"
    Write-CcLog "Låst til verifisert repository-commit: $ResolvedCommit"
    ...
    if ($manifest.stage -ne "stable") {
        throw "Command Center på main er ikke Stable. Automatisk pakkeoppdatering stoppes."
    }
