...
try{
  Write-CcLog "Starter eksplisitt RAH Command Center v1.7 pakkeoppdatering."
  ...
  if($manifest.version-ne"1.7.0"-or $manifest.stage-ne"stable"){throw "Command Center på main er ikke canonical v1.7 Stable."}
  ...
  if([string]$manifest.entry-ne"RAH-COMMAND-CENTER-V1.7.html"-or [string]$manifest.runtime-ne"rah-command-center-core-v1.7.js"){throw "Canonical entry/runtime er uventet."}
  Assert-FixedPackageContract -Manifest $manifest
  $releasePath=[string]$manifest.stable_release_manifest
  if($releasePath-ne"RAH-CC17-NODE13-STABLE-RELEASE.json"){throw "Canonical manifest peker ikke paa forventet Stable release."}
  ...
  Write-CcLog "Command Center v1.7 Stable klar fra verifisert commit $ResolvedCommit. Oppdatert: $updated. Uendret: $unchanged."
  Write-Host "RAH Command Center v1.7 Stable er klar." -ForegroundColor Green
