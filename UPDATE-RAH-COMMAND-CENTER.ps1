...
try{
  ...
  if($manifest.version-ne"1.8.0"-or $manifest.stage-ne"stable"){throw "Command Center på main er ikke canonical v1.8 Stable."}
  ...
  if([string]$manifest.entry-ne"RAH-COMMAND-CENTER-V1.8.html"-or [string]$manifest.runtime-ne"rah-command-center-core-v1.8.js"){throw "Canonical entry/runtime er uventet."}
  Assert-FixedPackageContract -Manifest $manifest
  $releasePath=[string]$manifest.stable_release_manifest
  if($releasePath-ne"RAH-CC18-NODE13-STABLE-RELEASE.json"){throw "Canonical manifest peker ikke paa forventet Stable release."}
  ...
  Write-CcLog "Command Center v1.8 Stable klar fra verifisert commit $ResolvedCommit. Oppdatert: $updated. Uendret: $unchanged."
  Write-Host "RAH Command Center v1.8 Stable er klar." -ForegroundColor Green
